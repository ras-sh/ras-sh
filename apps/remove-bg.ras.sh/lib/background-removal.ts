...
export const MODELS = {
  isnet_quint8: {
    id: "isnet_quint8" as const,
    name: "ISNet Quint8",
    description: "Fastest, quantized 8-bit",
    size: "~40MB",
  },
  isnet_fp16: {
    id: "isnet_fp16" as const,
    name: "ISNet FP16",
    description: "Balanced, 16-bit precision",
    size: "~80MB",
  },
  isnet: {
    id: "isnet" as const,
    name: "ISNet Full",
    description: "Highest quality, full precision",
    size: "~176MB",
  },
} as const;

// Cache for preloaded models
const preloadedModels = new Set<BackgroundRemovalModel>();
...
export async function processImageWithBackgroundRemoval(
  ...
  try {
    const { progress, model = "isnet_fp16" } = options;
    ...
    if (progress) {
      config.progress = (key, current, total) => {
        ...
        const progressPercent = Math.round(
          (current / total) * PercentageMultiplier
        );
